...
for axes, G in zip(axes_2d, [0.3, 3.0]):
    ...
    for ax, SYSTEM in zip(axes, ["sc", "bcc", "fcc"]):
        ...
        for PICK in range(min(len(purger.amplitude_eigenvalues), 4)):
            ...
            tsuji = -nu * xi / np.where(Delta != 0, Delta, np.inf)
            #ax.plot(xi, tsuji , dashes=[3.5, 3.5], 
            #        c="#009100", 
            #        label=r"$-\nu \xi / \Delta$" if PICK+1 == min(len(purger.amplitude_eigenvalues), 4) else None)
            ax.plot(xi, np.abs(tsuji - alpha) * np.abs(1 / alpha), 
                    c=colors[PICK],
                    label=PICK+1)
            ax.plot(xi, np.abs(tsuji - alpha) * np.abs(xi / alpha), dashes=[3.5, 3.5], 
                    c=colors[PICK],
