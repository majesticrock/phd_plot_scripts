...
U=0.1
SYSTEM = 'bcc'
main_df = load_all(f"lattice_cut/./T_C/{SYSTEM}/N={N}/", "all_gaps.json.gz", condition=[f"U={U}", f"E_F={E_F}"]).sort_values('g')
tc_df   = load_all(f"lattice_cut/./T_C/{SYSTEM}/N={N}/", "T_C.json.gz", condition=[f"U={U}", f"E_F={E_F}"]).sort_values('g')
