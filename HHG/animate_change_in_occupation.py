...
equilibrium_bot = main_df["lower_band"][0]

def update(frame):
    for coll in ax3d.collections[:]:
        coll.remove()

    data_top = main_df["upper_band"][frame] - equilibrium_top
    data_bot = main_df["lower_band"][frame] - equilibrium_bot
    facecolors_top = cmap(norm(data_top))
    facecolors_bot = cmap(norm(data_bot))

    ax3d.plot_surface(X, Z, Y_surf, facecolors=facecolors_top,
                      rstride=1, cstride=1, linewidth=0, antialiased=False)
    ax3d.plot_surface(X, Z, Y_surf_neg, facecolors=facecolors_bot,
                      rstride=1, cstride=1, linewidth=0, antialiased=False)
