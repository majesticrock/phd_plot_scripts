...
def run_and_plot(axes, axes_fft, params, color):
    ...
    df_A = data_loader.load_panda("HHG", f"{params['DIR']}/dgaussA_laser/{params['MODEL']}", "current_density.json.gz", 
    ...
    df_B = data_loader.load_panda("HHG", f"{params['DIR']}/dgaussB_laser/{params['MODEL']}", "current_density.json.gz", 
                      **hhg_params(T=params["T"], E_F=params["E_F"], v_F=params["v_F"], band_width=params["W"], 
                                   field_amplitude=1., photon_energy=1., 
                                   tau_diag=params["TAU_DIAG"], tau_offdiag=params["TAU_OFFDIAG"], t0=0))

    main_df = data_loader.load_panda("HHG", f"{params['DIR']}/dgauss_laser/{params['MODEL']}", "current_density.json.gz", 
                         **hhg_params(T=params["T"], E_F=params["E_F"], v_F=params["v_F"], band_width=params["W"], 
                                      field_amplitude=1., photon_energy=1., 
                                      tau_diag=params["TAU_DIAG"], tau_offdiag=params["TAU_OFFDIAG"], t0=0))
