...
df_A = data_loader.load_panda("HHG", f"exp_base/expA_laser/{MODEL}", "current_density.json.gz", 
                    **hhg_params(T=300, E_F=118, v_F=v_F, band_width=W, field_amplitude=1., photon_energy=1., tau_diag=30, tau_offdiag=-1, t0=0))
df_B = data_loader.load_panda("HHG", f"exp_base/expB_laser/{MODEL}", "current_density.json.gz", 
                    **hhg_params(T=300, E_F=118, v_F=v_F, band_width=W, field_amplitude=1., photon_energy=1., tau_diag=30, tau_offdiag=-1, t0=0))
...
for i, t0 in enumerate([0, 0.5, 1, 2, 3, 4, 6]):
    main_df = data_loader.load_panda("HHG", f"exp_{t0}/exp_laser/{MODEL}", "current_density.json.gz", 
                        **hhg_params(T=300, E_F=118, v_F=v_F, band_width=W, field_amplitude=1., photon_energy=1., tau_diag=30, tau_offdiag=-1, t0=0))
