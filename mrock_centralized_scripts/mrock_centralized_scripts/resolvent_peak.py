...
from .legacy_continued_fraction import continuum_edges, resolvent_data
...
class Peak:
    def __init__(self, data_folder, name_suffix, initial_search_bounds=(0., "lower_edge"), imaginary_offset=1e-6, xp_basis=True):
        ...
        if initial_search_bounds[1] == "lower_edge":
            upper_edge = continuum_edges(data_folder, name_suffix, xp_basis=xp_basis)[0]
        else:
            upper_edge = initial_search_bounds[1]
        
        data, data_real, w_lin, self.resolvent = resolvent_data(data_folder, name_suffix, lower_edge=initial_search_bounds[0], 
