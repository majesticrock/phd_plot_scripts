...
class ContinuedFraction:
    ...
    def denominator(self, w_param, with_terminator = True):
        w = w_param**2
        if with_terminator:
    ...
    def continued_fraction(self, w_param, with_terminator = True):
        w = w_param**2
        if with_terminator:
            G = w - self.A[len(self.A) - self.terminate_at] - self.B[len(self.B) - self.terminate_at] * self.terminator(w_param.real)
        else:
    ...
    def spectral_density(self, w_param, with_terminator = True):
        return NORM_FACTOR * self.continued_fraction(w_param, with_terminator).imag
    ...
    def mark_continuum(self, ax=None, scale_factor=1., label="Continuum"):
        ...
        if self.z_squared:
            plotter(scale_factor * np.sqrt(self.roots[0]), scale_factor * np.sqrt(self.roots[1]), zorder=-10, **args)
        else:
            plotter(scale_factor * self.roots[0], scale_factor * self.roots[1], zorder=-10, **args)
    ...
    def data_log_z(self, lower_edge=None, range=None, begin_offset=1e-6, number_of_values=20000, imaginary_offset=1e-6, with_terminator=True, reversed=False):
        ...
        data = self.continued_fraction( w_usage, with_terminator)
...
def resolvent_data_log_z(data_folder, name_suffix, lower_edge=None, range=None, begin_offset=1e-6, xp_basis=True, number_of_values=20000, imaginary_offset=1e-6, with_terminator=True, reversed=False, messages=True, ingore_first=40):
    ...
    if xp_basis:
        ...
        data = res.continued_fraction( w_usage, with_terminator)
    else:
        ...
        for idx, element in enumerate(element_names):
            ...
            def dos(w):
                if idx==0:
                    return res.continued_fraction(w, with_terminator)
                else:
                    return np.sqrt(w) * res.continued_fraction(w, with_terminator)
...
def resolvent_data(data_folder, name_suffix, lower_edge, upper_edge=None, xp_basis=True, number_of_values=20000, imaginary_offset=1e-6, with_terminator=True, use_start=True, messages=True, ingore_first=40):
    data = np.zeros(number_of_values, dtype=complex)
    if xp_basis:
        ...
        data = res.continued_fraction( w_lin, with_terminator)
    else:
        element_names = ["a", "a+b", "a+ib"]
        res = ContinuedFraction(data_folder, f"resolvent_{name_suffix}_{element_names[0]}", True, messages, ingore_first)
        if upper_edge is None:
...
def resolvent_in_continuum(data_folder, name_suffix, range=None, xp_basis=True, number_of_values=20000, imaginary_offset=1e-6, with_terminator=True, w_space=np.linspace):
    borders = continuum_edges(data_folder, name_suffix, xp_basis)
    if range is None:
        return resolvent_data(data_folder, name_suffix, borders[0], borders[1], xp_basis, number_of_values, imaginary_offset, with_terminator, w_space)
    else:
        return resolvent_data(data_folder, name_suffix, borders[0], borders[0] + range, xp_basis, number_of_values, imaginary_offset, with_terminator, w_space)